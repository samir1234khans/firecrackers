...
export class AudioEngine {
  ...
  async configure(enabled:boolean,volume:number,haptics:boolean,ambience:boolean) {
    ...
    this.haptics=haptics && typeof navigator.vibrate==='function'; this.volume=volume; this.ambient=ambience;
    if(!enabled) { this.enabled=false; this.stop(); return false; }
    try {
      if(!this.context) this.initialize();
      if(this.context!.state==='suspended') await this.context!.resume();
      if(this.disposed || this.context!.state!=='running') return false;
      this.enabled=true; this.master!.gain.setValueAtTime(this.volume,this.context!.currentTime);
      this.updateAmbience(); return true;
    } catch { this.enabled=false; this.stop(); return false; }
  }
  private initialize() {
    this.context=new AudioContext(); const c=this.context;
    this.master=c.createGain(); this.master.gain.value=0;
    this.compressor=c.createDynamicsCompressor(); this.compressor.threshold.value=-18;
    this.compressor.knee.value=18; this.compressor.ratio.value=7; this.compressor.attack.value=0.003; this.compressor.release.value=0.28;
    this.master.connect(this.compressor); this.compressor.connect(c.destination);
    this.noise=c.createBuffer(1,c.sampleRate*3,c.sampleRate); const values=this.noise.getChannelData(0),rand=randomStream(839721);
    let pink=0; for(let i=0;i<values.length;i++) { pink=(pink+rand()*0.04-0.02)/1.02; values[i]=(rand()*2-1)*0.65+pink*4; }
  }
  consume(events:SimEvent[],worldWidth:number) {
    for(const e of events) {
      ...
      if(e.type==='fuse') this.noiseVoice(at,1.5,1800,0.055,pan,'highpass');
      if(e.type==='launch') { this.noiseVoice(at,0.72,950,0.18,pan,'bandpass'); if(e.family===1) this.tone(at,0.65,940,510,0.018,pan); }
      if(e.type==='burst') {
        ...
        if(e.family===0 || e.family===3) for(let j=0;j<5;j++) this.noiseVoice(at+0.5+j*0.19,0.07,2400,0.035,pan,'highpass');
      }
      if(e.type==='crackle') for(let j=0;j<3;j++) this.noiseVoice(at+j*0.045,0.045,2600,0.045,pan,'highpass');
    }
  }
  private connect(source:AudioScheduledSourceNode,filter:AudioNode,at:number,duration:number,gain:number,pan:number) {
    if(!this.context || !this.master || this.voices.size>=40) { source.disconnect(); filter.disconnect(); return; }
    const c=this.context,g=c.createGain(),p=c.createStereoPanner(); p.pan.value=pan;
    g.gain.setValueAtTime(0,at); g.gain.linearRampToValueAtTime(gain,at+Math.min(0.015,duration/5));
    g.gain.exponentialRampToValueAtTime(0.0001,at+duration);
    source.connect(filter); filter.connect(g); g.connect(p); p.connect(this.master); this.voices.add(source);
    source.onended=()=>{ this.voices.delete(source); source.disconnect(); filter.disconnect(); g.disconnect(); p.disconnect(); };
    source.start(at); source.stop(at+duration+0.015);
  }
  private noiseVoice(at:number,duration:number,frequency:number,gain:number,pan:number,type:BiquadFilterType) {
    ...
    const s=this.context.createBufferSource(); s.buffer=this.noise; s.playbackRate.value=0.90+(Math.sin(at*7.31)+1)*0.1;
    const f=this.context.createBiquadFilter(); f.type=type; f.frequency.value=frequency; f.Q.value=0.65;
  ...
  private tone(at:number,duration:number,start:number,end:number,gain:number,pan:number) {
    ...
    const s=this.context.createOscillator(); s.type='sine'; s.frequency.setValueAtTime(start,at); s.frequency.exponentialRampToValueAtTime(end,at+duration);
  ...
  private updateAmbience() {
    ...
    if(this.ambient && this.enabled) {
      ...
      s.buffer=this.noise; s.loop=true; filter.type='lowpass'; filter.frequency.value=250; g.gain.value=0.017;
      s.connect(filter); filter.connect(g); g.connect(this.master); s.start(); this.ambience=s;
      s.onended=()=>{s.disconnect();filter.disconnect();g.disconnect();};
    }
  }
  stop() {
    if(this.master && this.context) { this.master.gain.cancelScheduledValues(this.context.currentTime); this.master.gain.setValueAtTime(0,this.context.currentTime); }
