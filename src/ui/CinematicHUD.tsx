...
const FAMILY_META: Record<FamilyId, FamilyMeta> = {
  'gold-willow': {
    mood: 'Elegant · Lingering',
    description: 'A slow golden canopy that hangs, falls and leaves a warm afterglow.',
    metrics: [84, 72, 96, 86],
  },
  'multicolor-peony': {
    mood: 'Vibrant · Joyful',
    description: 'A crisp spherical break with jewel-like colour and a cleaner fade.',
    metrics: [82, 80, 55, 92],
  },
  'chrysanthemum': {
    mood: 'Bold · Radiant',
    description: 'Dense copper rays with stronger trailing structure and late curvature.',
    metrics: [88, 84, 74, 88],
  },
  'silver-crossette-crackle': {
    mood: 'Sharp · Crackling',
    description: 'Silver parents travel, split spatially and finish with restrained crackle.',
    metrics: [76, 78, 62, 70],
  },
  'grand-finale': {
    mood: 'Epic · Layered',
    description: 'Visible carriers build a finite multi-stage sequence with a golden ending.',
    metrics: [98, 94, 90, 98],
  },
};

const METRICS = ['Intensity', 'Height', 'Duration', 'Spread'] as const;

function Meter({ value }: { value: number }) {
  return <span className='hud-meter' aria-hidden='true'>
    {Array.from({ length: 8 }, (_, index) => <i key={index} className={index < Math.round(value / 12.5) ? 'filled' : ''}/>)}
  </span>;
}
...
export function CinematicHUD({
  ...
  return <>
    <header className='hud-command chrome' aria-label='Firecrackers command deck'>
      <div className='hud-brand'>
        <span className='hud-brand-mark' aria-hidden='true'><Sparkles size={24} strokeWidth={1.35}/></span>
        <span><strong>Firecrackers</strong><small>Cinematic fireworks</small></span>
      </div>
    ...
    <aside className='hud-inspector glass chrome' aria-label={`${selected.name} details`}>
      ...
      <div className='hud-metrics'>
        {METRICS.map((metric, index) => <div key={metric}><span>{metric}</span><Meter value={meta.metrics[index]}/></div>)}
      </div>
    </aside>

    <section className='hud-deck-wrap chrome' aria-label='Firework controls'>
      ...
      <div className='hud-deck glass'>
        ...
        <div className='hud-action-row'>
          ...
          <div className='hud-ignite-shell'>
            <button
              className={`hud-ignite${holding ? ' holding' : ''}`}
              aria-label='Hold to light selected firework'
              disabled={!canLight}
              style={holdStyle}
              ...
              onKeyUp={keyUp}
            >
              <span className='hud-ignite-core'><Flame size={22}/><strong>{holding ? 'Release to cancel' : 'Hold to ignite'}</strong><small>{holding ? 'Fuse contact' : 'Tap & hold'}</small></span>
            </button>
          </div>
