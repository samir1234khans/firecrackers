...
export default function App() {
  const [prefs,setPrefs]=useState(loadPreferences),[overlay,setOverlay]=useState<Overlay>(()=>loadPreferences().onboarded?null:'help');
  const [epoch,setEpoch]=useState(0),[hidden,setHidden]=useState(false),[notice,setNotice]=useState('');
  ...
  const world=useWorld(host,prefs,epoch,notify),platform=usePlatform(notify),state=world.snapshot;
  const worldRef=useRef(world);worldRef.current=world;
  const context=useRef({overlay,prefs,state});context.current={overlay,prefs,state};
  ...
  const open=(next:Overlay)=> {world.pause(true);platform.releaseWake();setOverlay(next);wake();};
  const close=()=> {setOverlay(null);world.pause(false);wake();};
  const select=(id:FamilyId)=> {if(world.sim.current.select(id))change('family',id);world.refresh();setNotice('');wake();};
  useEffect(()=>{savePreferences(prefs);},[prefs]);
  useEffect(()=>{if(state.bursts>0&&!prefs.onboarded)change('onboarded',true);},[state.bursts,prefs.onboarded,change]);
  useEffect(()=> {
    const id=setInterval(()=>{const c=context.current;if(Date.now()-lastActivity.current>3000&&!c.overlay&&!keyboard.current&&!c.state.holding&&c.prefs.onboarded&&c.state.launched>0&&!c.state.paused)setHidden(true);},500);
    return ()=>clearInterval(id);
  },[]);
  useEffect(()=> {
    const onKey=(event:KeyboardEvent)=> {
      const c=context.current;keyboard.current=true;lastActivity.current=Date.now();setHidden(false);
      if(c.overlay)return;
      if(event.key==='Escape'){event.preventDefault();return;}
      if(event.target instanceof HTMLElement && event.target.closest('button,input,select,textarea'))return;
      const w=worldRef.current;
      if(event.key===' '){event.preventDefault();w.pause(!w.sim.current.paused);}
      if(event.repeat)return;
      if(event.key.toLowerCase()==='l')w.ignite();
      if(/^[1-5]$/.test(event.key)){const f=FAMILIES[Number(event.key)-1];if(w.sim.current.select(f.id))change('family',f.id);w.refresh();}
      if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();w.sim.current.stopShow(false);w.sim.current.setPlacement(w.sim.current.placement+(event.key==='ArrowLeft'?-0.05:0.05));w.refresh();}
  ...
  const reset=()=>{world.reset();void world.configureSound(false);platform.releaseWake();setPrefs(defaults());setNotice('');setOverlay('help');setHidden(false);};
  return <main className={`fireworks-app${hidden?' controls-hidden':''}${prefs.reducedMotion?' reduced-motion':''}`} onPointerMove={wake} onPointerDownCapture={event=>{keyboard.current=false;if(hidden&&!(event.target as HTMLElement).closest('[data-always]')){event.preventDefault();event.stopPropagation();wake();}else lastActivity.current=Date.now();}} onFocusCapture={wake}>
    ...
    {world.ready&&!world.error&&<div className='placement-stage chrome' style={{left:`${state.placement*100}%`}}>
      {canLight&&<>
        <button className='move-target' aria-label='Drag firework left or right' title='Drag to place' onPointerDown={event=>{if(event.button!==0)return;dragging.current=true;world.sim.current.stopShow(false);event.currentTarget.setPointerCapture(event.pointerId);}} onPointerMove={event=>{if(dragging.current)setPlacement(event.clientX/window.innerWidth);}} onPointerUp={()=>{dragging.current=false;}} onPointerCancel={()=>{dragging.current=false;}}/>
        <button className={`fuse-target${state.holding?' holding':''}`} aria-label='Hold to light selected firework' style={{'--hold':`${Math.min(1,state.holdProgress)*100}%`} as React.CSSProperties} onPointerDown={event=>{if(event.button!==0)return;event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);world.sim.current.beginHold();world.refresh();}} onPointerUp={()=>{world.sim.current.cancelHold();world.refresh();}} onPointerCancel={()=>{world.sim.current.cancelHold();world.refresh();}} onLostPointerCapture={()=>{world.sim.current.cancelHold();world.refresh();}} onKeyDown={event=>{if(event.key==='Enter'){event.preventDefault();event.stopPropagation();if(!event.repeat)world.ignite();}else if(event.key===' '){event.preventDefault();event.stopPropagation();if(!event.repeat)world.sim.current.beginHold();}}} onKeyUp={event=>{if(event.key===' '){event.preventDefault();world.sim.current.cancelHold();world.refresh();}}}><Flame size={18}/></button>
        <span className='fuse-hint'>{state.holding?'Release to cancel':'Hold the fuse to light'}</span>
      </>}
    </div>}
    <div className='dock-wrap chrome'>
      <div className='scene-message' role='status' aria-live='polite'>{notice||state.message}</div>
    ...
    <div className='reveal-controls' aria-hidden={!hidden}>{hidden&&<><button className='icon-button glass' aria-label='Show controls' onClick={wake}><Settings2 size={18}/></button><button className='icon-button glass' aria-label='Pause scene' data-always='true' onClick={resumeOrPause}><Pause size={18}/></button></>}</div>
    ...
    {overlay==='settings'&&<Dialog title='Make yourself comfortable.' onClose={close}>
      ...
      <details className='diagnostics'><summary>Graphics details</summary><dl><div><dt>Renderer</dt><dd>{world.backend}</dd></div><div><dt>Active quality</dt><dd>{state.quality}</dd></div><div><dt>Visible particles</dt><dd>{state.particles.toLocaleString()}</dd></div><div><dt>Smoke layers</dt><dd>{state.smoke} / 160</dd></div><div><dt>Launched / bursts</dt><dd>{state.launched} / {state.bursts}</dd></div></dl></details>
